...
public class CreateTableParserListener extends TableCommonParserListener {
    ...
    @Override
    public void enterTableOptionWithSystemVersioning(MariaDBParser.TableOptionWithSystemVersioningContext ctx) {
        parser.runIfNotNull(() -> tableEditor.addAttribute(Attribute.editor()
                .name(MariaDbAntlrDdlParser.SYSTEM_VERSIONED_TABLE_ATTRIBUTE)
                .value(true)
                .create()), tableEditor);
        super.enterTableOptionWithSystemVersioning(ctx);
    }
}
