...
class BinlogDmlFailureLoggingTest {
    ...
    @Test
    void shouldUseDedicatedLoggerWhenDebeziumParentOnlyAllowsErrors() {
        ...
        try {
            ...
            new DmlFailureLogger(new FailureLogLimiter()).warnUnknownTable(TABLE_ID, Envelope.Operation.CREATE,
                    Collections.singletonMap("pos", 100), "mariadb-bin.000001", 100, 120);
    ...
    @Test
    void shouldRateLimitUnknownTableWarningAndSummarizeSuppressedEvents() {
        final AtomicLong time = new AtomicLong();
        final FailureLogLimiter limiter = new FailureLogLimiter(1024, 20, WINDOW, EXPIRY, time::get);
        final String loggerName = "io.debezium.connector.binlog.dml.failure.test";
    ...
    @Test
    void shouldLogBoundedFatalSchemaRowSizeContext() {
        final String loggerName = "io.debezium.connector.binlog.dml.failure.mismatch";
        final LogInterceptor interceptor = new LogInterceptor(loggerName);
        final DmlFailureLogger failureLogger = new DmlFailureLogger(LoggerFactory.getLogger(loggerName), new FailureLogLimiter());
        ...
        assertThat(interceptor.getLogEntriesThatContainsMessage("category=SCHEMA_ROW_SIZE_MISMATCH"))
                .singleElement()
                .satisfies(message -> assertThat(message)
                        .contains("action=STOP", "operation=UPDATE", "internalSchemaSize=2", "rowSize=1",
